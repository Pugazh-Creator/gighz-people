...
<body>
    <?= view('navbar/sidebar') ?>
    <div class="container permission-body">
        ...
        <div class="leaverequest_tools">
            <!-- Search Form -->
            <form method="GET" action="<?= site_url('hrcontroller/getallpermission'); ?>">
            ...
            <form method="GET" action="<?= site_url('hrcontroller/getallpermission'); ?>">
                <div class="leaveRequest_sorting">
                    <div class="leaveRequest_sortby">
                        <select name="sort_by">
                            <option value="permission_user_id" <?= ($sortBy == 'permission_user_id') ? 'selected' : ''; ?>>Default</option>
                        ...
                        <tbody>
                            <?php foreach ($datas as $compen): ?>
                                ...
                                <tr>
                                    ...
                                    <td><?= ucfirst($compen['permission_status']) ?></td>
                                    <td class="actions">
                                        <a class="approve" href="<?= base_url('/hrcontroller/changepermissionstatus') ?>/<?= $compen['permission_id'] ?>/approved/<?= $compen['permission_user_id'] ?>/"><i class='bx bx-check'></i></a> |
                                        <a class="reject" href="<?= base_url('/hrcontroller/changepermissionstatus') ?>/<?= $compen['permission_id'] ?>/rejected/<?= $compen['permission_user_id'] ?>/"><i class='bx bx-x'></i></a> |
                                        <a class="delete" href="<?= base_url('/hrcontroller/changepermissionstatus') ?>/<?= $compen['permission_id'] ?>/delete/<?= $compen['permission_user_id'] ?>/"><i class='bx bx-trash'></i></a>
                                    </td>
                                </tr>
...
<?php else : ?>
    <h3>No Permission Requests Found</h3>
<?php endif; ?>
