...
// Connect to eSSL Biometric Device
...
async function calculateEmployeeHours() {
    ...
    try {
        ...
        //  console.log('hello');
         await zkInstance.createSocket(); // Connecting to biometric device
        // console.log("✅ Connected to eSSL Biometric Device!");
 
        // Fetch all employees
        const [users] = await connection.execute('SELECT * FROM attendance_users');
        // console.log('Attendance User Connected Success Fully');
        ...
        for (const user of users) {
            ...
            if (employee_status == '1' && emp_id != null) {
                ...
                startDate = new Date(startDate.setDate(startDate.getDate() + 1)).toISOString().split('T')[0]; // Ensure it starts from the next day
                let yesterday = new Date();
                yesterday = new Date(yesterday.setDate(yesterday.getDate() - 1)).toISOString().split('T')[0]; // Process until yesterday


                // console.log(`start Date = ${startDate} : End Date = ${new Date(yesterday)}`)
                while (new Date(startDate) <= new Date(yesterday) || new Date(yesterday) == new Date(startDate)) {
                    ...
                    // console.log('Formated dates : '+ formattedDate);
                    ...
                    if (result.length > 0) {
                        ...
                        await connection.execute(
                            'INSERT INTO attendance (user_id, date, total_hours, work_status) VALUES (?, ?, ?, ?)',
                            [user_id, formattedDate, totalHours, workType]
                        );
                    }
        ...
        console.log('❌ Error to Proccessing Data');
