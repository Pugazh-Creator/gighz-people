...
<body>
    ...
    <section class="container leaverequest">
        <div class="availabel-leave">
            <h1>Balance Leave : <?= $total_leaves; ?></h1>
            <form method="GET" action="<?= site_url('empleaves')  ?>">
                <select name="year" id="year">
                    <option value="">Default</option>
                    <?php for ($i = 23; $i < date('y'); $i++) : ?>
                        ...
                        <tbody>
                            <?php foreach ($data as $leaves): ?>
                                <tr>
                                    ...
                                    <td> <?= esc($leaves['name']); ?></td>
                                    <td><?= esc($leaves['leave_type']); ?></td>
                                    <td><?= esc($leaves['start_date']); ?></td>
                                    <td><?= esc($leaves['end_date']); ?></td>
                                    <td><?= esc($leaves['total_num_leaves']); ?></td>
                                    <!-- <td><?php //esc($leaves['balence_leave']); ?></td>
                                    <td> <?php // esc($leaves['remaining_leaves']);?></td> -->
                                    <td class="td-reason" onclick="showFullText(this)"><?= esc($leaves['reason']); ?></td>
                                    <td><?= esc($leaves['status']); ?></td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
